  ...
  images.forEach(img => {
    ...
    if (img.type === 'gallery' && img.images?.length) {
      ...
      const slides = img.images.map((im, i) =>
        ...
        `<div class="img-wrap"><img src="${escapeHtml(im.url)}" class="entry-img" loading="lazy" decoding="async" alt="">` +
      ...
      html = img.url.endsWith('.gif')
        ? `<img src="${escapeHtml(img.url)}" class="entry-img" loading="lazy" decoding="async" alt="">`
